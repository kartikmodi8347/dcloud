        ...
        <div class="row" style="margin-left:2%; ">
          ...
          <div class="col-l15" style="margin-bottom: 60px; ">
            ...
            <div class="card circul-x2 full-on-s" style="width: 100%; margin-top:20px !important; height: auto; margin: 20px 10px 0px 10px ; min-height: 200px; background: rgba(255,255,255,0.6)" >
              <div class="card-head">Received Files</div>
              <div style="padding: 15px; overflow-x: auto;">
              <table width="100%" style="font-size: 14px;">
                <tr>
                  <th width="35" colspan="2">Shared By</th>
                  <th>Title</th>
                  <th width="150">Date-Time</th>
                  <th width="50"></th>
                </tr>
                <?php 
                $shared_q = mysqli_query($con,"SELECT * FROM share_to WHERE share_user='$user' ");
                $s_num = mysqli_num_rows($shared_q);
                if($s_num > 0)
                {
                  while ($shared_row = mysqli_fetch_assoc($shared_q)) 
                  {
                      $share_info_q = mysqli_query($con,"SELECT * FROM share_privacy WHERE share_id='$shared_row[share_id]' ");
                      $share_info_num = mysqli_num_rows($share_info_q);
                      if ($share_info_num > 0)
                      {
                        $share_info = mysqli_fetch_assoc($share_info_q);
                        $sered_u_q = mysqli_query($con,"SELECT first_name,last_name,profile_pic FROM user_reg WHERE id='$shared_row[id]' ");
                        $sered_u = mysqli_fetch_assoc($sered_u_q);

                        if ($sered_u['profile_pic'] == "")
                        {
                          $pro = '../img/profile.png';
                        }
                        else
                        {
                          $pro = 'profile_pic/'.$shared_row['id'].'/'.$sered_u['profile_pic'];
                        }
                     echo '<tr class="link-row" data-href="share_view/0/'.$shared_row['share_id'].'">';
                        echo'<td width="35">'.'<img src="'.$pro.'" style="height:35px; width:35px; border-radius:50%;"/>'.'</td>';
                        echo'<td width="140">'.$sered_u['first_name'].' '.$sered_u['last_name'].'</td>';
                        echo'<td>'.$share_info['share_title'].'</td>';
                        echo'<td>'.date('d M Y h:i a',$share_info['time']).'</td>';
                        echo'<td></td>';
                      echo'</tr>';
                ...
                {
                  echo '<div class="blue lighten-5" style="line-height: 35px; text-align: center;font-size: 14px;">You have not recived file yet</div>';
                } 
                ?>
