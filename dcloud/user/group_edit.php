...
<body>
  ...
  include("header.php");
	...
	if(isset($_GET['grp_id']))
	{
		...
		if ($grp_num == 0)
		{
      $admin = mysqli_query($con,"SELECT * FROM `group_admin` WHERE group_id='$_GET[grp_id]' AND admin_id=$user");
...
*/
        if($create)
        {
          echo '<meta http-equiv="refresh" content="0;url=http://localhost/dclud/user/group/'.$_GET['grp_id'].'"/>';
        }
      }
      ?>
