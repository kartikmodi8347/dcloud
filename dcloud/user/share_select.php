...
$dir_own = mysqli_query($con,"SELECT id FROM directory WHERE id=$user AND dir_id=$dir ");
$error = '';
if (mysqli_num_rows($dir_own) == 0)
{
	$error = "<div class='red white-text' style='padding:20px 0 20px 0; text-align:center;'>Sorry, you can't share this file</div>";
}
...
if(dir_path($dir))
{
	$real_dir_path = dir_path($dir);
}
else
{
	$real_dir_path = dirinfo($dir);
}            
?>
<body>
       ...
       <div class="row">
        	<div class="col-l20">
          		<div class="card d-depth-2 circul-x1 full-on-l full-on-s" style="margin-top: 40px !important; ">
					...
					<div>
					<?php echo '<p style="font-size:14px; line-height:28px; margin-left:8px;">Current Folder : <a href="file_area.php?dir='.$_GET['dir'].'"><b>'.dirinfo($dir).'</b></a></p>'; ?>
						<form method="POST">
						...
						if ($error == '')
						{
							...
							if(mysqli_num_rows($select_q) > 0)
							{
										...
										while ($dir = mysqli_fetch_assoc($select_q))
										{
											...
											echo '<tr>
													...
													<input type="checkbox" id="folder" name="dir_ids[]" value="<?php echo $dir['linked_dir_id']; ?>"/>
							...
							$select_file = mysqli_query($con,"SELECT * FROM docfile WHERE dir_id='$_GET[dir]' ");
							...
							if(mysqli_num_rows($select_file) > 0)
							{
										while ($doc = mysqli_fetch_assoc($select_file))
										{
											...
											echo '<tr>
													...
													{
													echo'<img src="../data/'.$user.'/'.$real_dir_path.'/'.$doc['doc_path'].'" style="width:auto; height:25px; max-width:25px;"/>';
													}
							...
							if (mysqli_num_rows($select_file) == 0 AND mysqli_num_rows($select_q) == 0)
							{
								echo "<div class='red white-text' style='padding:20px 0 20px 0; text-align:center;'>No document or sub folder exist in this folder</div>";
							}
							...
							if(isset($folder) || isset($files))
							{
								...
								else
								{
  							    ...
  							    if ($share_p =='Public')
								{
									echo '<meta http-equiv="refresh" content="0;url=./user/public_share/'.$share_id.'"/>';
								}
								if ($share_p =='Group')
								{
